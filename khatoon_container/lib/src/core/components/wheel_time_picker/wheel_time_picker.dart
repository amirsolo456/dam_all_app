...
class _ScrollTimePickerWheelState extends State<ScrollTimePickerWheel> {
  ...
  void _initializePickers() {
    final int currentHour = DateTime.now().hour % 12;
    final int currentMinute = DateTime.now().minute;
    final int ampmIndex = DateTime.now().hour >= 12 ? 1 : 0;
  ...
  void _onAmpmChanged(int index) {
    setState(() {
      final int currentHour = _selectedTime.hour;
      ...
      child: Row(
        ...
        children: <Widget>[
          ...
          _buildPicker(
              ...
              60,
              _onMinuteChanged,
                  (int index) => index.toString().padLeft(2, '0'),
              'MM',
              BorderRadius.zero),
          _buildPicker(
              _ampmController,
